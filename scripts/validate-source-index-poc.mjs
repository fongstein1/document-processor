...
const main = async () => {
  ...
  const expectedPromotedChunkCount = [...promotedSourceIds].reduce((sum, sourceId) => { const source = config.sources.find((candidate) => candidate.sourceId === sourceId); return sum + (source?.expectedChunkCount ?? source?.chunks?.length ?? 0) }, 0)
  if (repositoryManifest.extensions?.promotionDecisionPaths?.length !== promotionDecisionRecords.length || repositoryManifest.extensions?.promotedSourcePackageCount !== promotedSourceIds.size || repositoryManifest.extensions?.promotedChunkCount !== expectedPromotedChunkCount) fail('Repository manifest does not record all scope-specific canonical promotions.')
