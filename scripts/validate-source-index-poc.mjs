...
const main = async () => {
  ...
  if (vm30SourcePackage.processing?.reviewOnly !== true || vm30SourcePackage.processing?.promotionStatus !== 'not_promoted' || vm30SourcePackage.chunks.length !== 51 || vm30SourcePackage.chunks.filter((chunk) => chunk.chunkLevel === 'parent').length !== 8 || vm30SourcePackage.chunks.filter((chunk) => chunk.chunkLevel === 'child').length !== 43 || vm30SourcePackage.chunks.filter((chunk) => chunk.retrievalEligible).length !== 42 || vm30SourcePackage.chunks.some((chunk) => chunk.promotionEligible !== false)) fail('VM-30 canonical review metadata or hierarchy is incomplete.')
  ...
  if (repositoryManifest.extensions?.promotionDecisionPaths?.length !== promotionDecisionRecords.length || repositoryManifest.extensions?.promotedSourcePackageCount !== 8 || repositoryManifest.extensions?.promotedChunkCount !== 331) fail('Repository manifest does not record all scope-specific canonical promotions.')
