...
const main = async () => {
  ...
  if (inventory.summary.canonicalSourcePackages !== 25 || inventory.summary.canonicalChunks !== 749) fail('Inventory snapshot does not include the current VM-22 candidate package.')
  if (inventory.summary.promotedCanonicalPackages !== 10 || inventory.summary.promotedCanonicalChunks !== 460) fail('Inventory does not reflect the scope-specific VM-01, VM-20, VM-21, VM-30, and VM-31 promotions.')
  ...
  if (inventory.summary.vm21StructuralParents !== 15 || inventory.summary.vm21ChildUnits !== 63 || inventory.summary.vm21PromotionStatus !== 'promoted' || inventory.summary.vm21RelationshipCandidates !== 20) fail('Inventory does not reflect the promoted VM-21 hierarchy and separately governed relationship registry.')
  const vm21Target = inventory.corpusTargets.find((target) => target.targetId === 'vm-21')
  if (vm21Target?.assessment !== 'canonical_complete' || !vm21Target.evidenceSourceIds.includes('vm21-current-manual')) fail('VM-21 target does not record its promoted canonical package.')
  if (inventory.summary.vm22StructuralParents !== 15 || inventory.summary.vm22ChildUnits !== 68 || inventory.summary.vm22PromotionStatus !== 'not_promoted' || inventory.summary.vm22RelationshipCandidates !== 111) fail('Inventory does not reflect the review-only VM-22 hierarchy and separately governed relationship registry.')
  const vm22Target = inventory.corpusTargets.find((target) => target.targetId === 'vm-22')
  if (vm22Target?.assessment !== 'canonical_review_candidate' || !vm22Target.evidenceSourceIds.includes('vm22-current-manual')) fail('VM-22 target does not record its review-only canonical candidate.')
  if (inventory.summary.vm30StructuralParents !== 8 || inventory.summary.vm30ChildUnits !== 43 || inventory.summary.vm30PromotionStatus !== 'promoted' || inventory.summary.vm30RelationshipCandidates !== 16) fail('Inventory does not reflect the promoted VM-30 hierarchy and separately governed relationship registry.')
  const vm30Target = inventory.corpusTargets.find((target) => target.targetId === 'vm-30')
  if (vm30Target?.assessment !== 'canonical_complete' || !vm30Target.evidenceSourceIds.includes('vm30-current-manual')) fail('VM-30 target does not record its promoted canonical package.')
