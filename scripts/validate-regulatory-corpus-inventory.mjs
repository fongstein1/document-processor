...
const main = async () => {
  ...
  if (inventory.summary.canonicalSourcePackages !== 24 || inventory.summary.canonicalChunks !== 666) fail('Inventory snapshot does not include the complete current VM-21 review package; regenerate intentionally.')
  if (inventory.summary.promotedCanonicalPackages !== 9 || inventory.summary.promotedCanonicalChunks !== 382) fail('Inventory does not reflect the scope-specific VM-01, VM-20, VM-30, and VM-31 promotions.')
  ...
  if (inventory.summary.vm21StructuralParents !== 15 || inventory.summary.vm21ChildUnits !== 63 || inventory.summary.vm21PromotionStatus !== 'not_promoted' || inventory.summary.vm21RelationshipCandidates !== 20) fail('Inventory does not reflect the review-only VM-21 hierarchy and separately governed relationship registry.')
  const vm21Target = inventory.corpusTargets.find((target) => target.targetId === 'vm-21')
  if (vm21Target?.assessment !== 'canonical_review_candidate' || !vm21Target.evidenceSourceIds.includes('vm21-current-manual')) fail('VM-21 target does not record its review-only canonical candidate.')
  ...
  if (inventory.summary.candidateRelationships !== 180 || inventory.summary.promotedRelationships !== 0) fail('Relationship governance counts are inconsistent with the review-only Reg-213, VM-01, VM-21, VM-30, and VM-31 registries.')
