...
const main = async () => {
  ...
  if (inventory.summary.canonicalSourcePackages !== 23 || inventory.summary.canonicalChunks !== 541) fail('Inventory snapshot does not include the complete current VM-31 review package; regenerate intentionally.')
  ...
  if (inventory.summary.vm31StructuralParents !== 9 || inventory.summary.vm31ChildUnits !== 75 || inventory.summary.vm31PromotionStatus !== 'not_promoted' || inventory.summary.vm31RelationshipCandidates !== 92) fail('Inventory does not reflect the review-only VM-31 hierarchy and relationship registry.')
  const vm31Target = inventory.corpusTargets.find((target) => target.targetId === 'vm-31')
  if (vm31Target?.assessment !== 'canonical_review_candidate' || !vm31Target.evidenceSourceIds.includes('vm31-current-manual')) fail('VM-31 target does not record its review-only canonical package.')
  ...
  if (inventory.summary.candidateRelationships !== 144 || inventory.summary.promotedRelationships !== 0) fail('Relationship governance counts are inconsistent with the review-only Reg-213, VM-01, and VM-31 registries.')
