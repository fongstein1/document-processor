...
const codeReviewPromptPath = path.join(processedRoot, 'review_packages', 'vm01-retrieval-code-review-prompt.md')
...
const main = async () => {
  ...
  const expectedImplementationSources = new Set([
    'scripts/evaluate-source-index-retrieval.mjs',
    'scripts/evidence-sufficiency.mjs',
    'scripts/test-vm20-support-gate.mjs',
    'scripts/validate-vm01-definitions.mjs',
  ])
  assert(implementationReview.artifactPurpose === 'narrow_independent_code_review' && implementationReview.fileCount === expectedImplementationSources.size, 'VM-01 implementation review manifest is incomplete.')
  for (const file of implementationReview.files) {
    assert(expectedImplementationSources.delete(file.sourcePath), `Unexpected or duplicate VM-01 implementation source snapshot: ${file.sourcePath}.`)
    const sourcePath = path.join(repoRoot, ...file.sourcePath.split('/'))
    const snapshotPath = path.join(repoRoot, ...file.snapshotPath.split('/'))
