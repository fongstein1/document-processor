...
for(const name of await fs.readdir(publicRoot))if(name.endsWith('.json'))validateGitSafeArtifact({artifactType:name,value:await read(path.join(publicRoot,name))})
