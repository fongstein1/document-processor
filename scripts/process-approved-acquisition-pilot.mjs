...
const externalProcessingRoot = path.join('C:\\Dev\\Document Processor Sources', '_processed-private', processingRunId)
...
const corpusSmokeTestsFor = (record, allChunks, allProcessed) => {
  ...
  const numericCell = numericEvidence?.cellRef || numericChunk?.extensions?.tableBlock?.cellRefs?.[0]
  ...
  const cases = [
    { kind: 'title_identity', query: record.documentTitle, expectedCandidateId: record.candidateId, expectedMatch: true, expectedRegion: 'source identity', rankThreshold: 1, expectedSourceSha256: record.SHA256, expectedAuthorityRole: record.disposition, rationale: 'Title query is ranked against the complete seven-source corpus and must resolve to this source.' },
    ...
    { kind: 'wrong_source_empty_match', query: 'qzxv-blorf-mivak-94731', expectedCandidateId: null, expectedMatch: false, expectedRegion: 'no matching corpus evidence', rankThreshold: 1, rationale: 'A nonsense token remains a basic no-match control and is not counted as confusion-set coverage.' },
  ]
  for (const slot of confusionSlots[record.pilotSlot] || []) {
...
const main = async () => {
  ...
  const checks = {
    ...
    confusionSetNegatives: smokeTests.flatMap((source) => source.tests).filter((test) => test.testKind === 'wrong_source_topic_negative').length >= 7 && smokeTests.flatMap((source) => source.tests).filter((test) => test.testKind === 'wrong_source_topic_negative').every((test) => test.pass && test.wrongSourceExcluded),
