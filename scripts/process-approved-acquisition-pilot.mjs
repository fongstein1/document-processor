...
import { sha256Bytes, substantiveArtifactTypes } from './rights-storage.mjs'
...
const batchId = argValue('--batch-id', 'acquisition-pilot-2026-09-02')
const processingRunId = argValue('--processing-run-id', 'processing-' + batchId)
const outputRoot = path.resolve(argValue('--output-root', path.join(repoRoot, 'data', 'processed', 'review_packages', batchId)))
const externalProcessingRoot = path.resolve(argValue('--external-root', path.join('C:\\Dev\\Document Processor Sources', '_processed-private', processingRunId)))
const rightsStatus = 'RIGHTS_REVIEW_REQUIRED'

if (externalProcessingRoot.toLowerCase().includes('onedrive')) throw new Error('OneDrive is prohibited for scaled processing artifacts.')
...
const writeExternalJson = async (externalRoot, sourceId, artifactType, value, source, generatedAt) => {
  ...
  const externalPath = path.join(externalRoot, sourceId, fileName)
  const bytes = Buffer.from(JSON.stringify(value, null, 2) + '\n', 'utf8')
  await fs.mkdir(path.dirname(externalPath), { recursive: true })
  await fs.writeFile(externalPath, bytes)
  return { processingRunId, sourceId, artifactType, externalPath, sha256: sha256Bytes(bytes), byteCount: bytes.length, sourceRawSha256: source.fileHash, generatedTimestamp: generatedAt, rightsStorageStatus: source.rightsStatus, reviewOnly: true }
}
...
with zipfile.ZipFile(path) as package:
    ...
    sheets = []
    package_external_links = sorted([name for name in package.namelist() if name.startswith("xl/externalLinks/") and not name.endswith(".rels")])
...
const makeChunks = (record, extraction, metadata) => {
  ...
  const add = (text, locator, pageStart, pageEnd, citationText, structured, details = {}) => {
    ...
    chunks.push({
      ...
      extensions: structured ? { worksheet: details.worksheet, sheetState: details.sheetState, mergedRanges: details.mergedRanges || [], tableBlock: details.tableBlock, duplicateLabelValues: details.duplicateLabelValues || [], contentInventory: details.contentInventory || null, exceptionCode: details.exceptionCode || null, authoritySupportRole: record.disposition, sourceSha256: record.SHA256 } : { coveredPages: details.coveredPages || [pageStart], emptyPageDetails: details.emptyPages || [], authoritySupportRole: record.disposition, sourceSha256: record.SHA256 },
    })
  }
...
const confusionSlots = { A3: ['S2'], S2: ['A3'], A4: ['S3'], S3: ['A4'], A2: ['S1'], S1: ['A2', 'A3'], S4: ['A2', 'S1'] }
const getConfusionSlots = (record, allProcessed) => {
  if (confusionSlots[record.pilotSlot]) return confusionSlots[record.pilotSlot]
  const sameFamily = allProcessed.filter((item) => item.record.candidateId !== record.candidateId && item.metadata.sourceFamilyId === slugify(record.family).replace(/-/g, '_') && item.chunks.length)
  if (!sameFamily.length) return []
  const familyRecords = allProcessed.filter((item) => item.metadata.sourceFamilyId === slugify(record.family).replace(/-/g, '_') && item.chunks.length)
  const currentIndex = familyRecords.findIndex((item) => item.record.candidateId === record.candidateId)
  return [familyRecords[(currentIndex + 1) % familyRecords.length].record.pilotSlot]
}
const corpusSmokeTestsFor = (record, allChunks, allProcessed) => {
  ...
  const titleTerms = allProcessed.map((item) => new Set(tokens(item.record.documentTitle)))
  const genericTitleTerms = new Set(['current', 'edition', '2026', '2015', 'rates', 'report', 'manual', 'publication', 'statement', 'blank', 'valuation'])
  const distinctive = tokens(record.documentTitle).find((term) => !genericTitleTerms.has(term) && titleTerms.filter((terms) => terms.has(term)).length === 1) || firstTerms.find((term) => !genericTitleTerms.has(term)) || slugify(record.documentTitle).split('-')[0]
  ...
  const numericEvidence = allProcessed.find((item) => item.record.candidateId === record.candidateId)?.numericEvidence
  const firstNumericChunk = record.expectedFileType === 'XLSX' ? chunks.find((chunk) => chunk.extensions?.tableBlock?.cellRefs?.length) : chunks[chunks.length - 1]
  const numericCell = numericEvidence?.cellRef || firstNumericChunk?.extensions?.tableBlock?.cellRefs?.[0]
  const numericChunk = record.expectedFileType === 'XLSX' ? chunks.find((chunk) => chunk.extensions?.tableBlock?.cellRefs?.includes(numericCell)) || firstNumericChunk : firstNumericChunk
  const numericQuery = record.expectedFileType === 'XLSX' ? [record.documentTitle, numericEvidence?.sheetName, numericCell, numericEvidence?.value, numericEvidence?.formula].filter(Boolean).join(' ') : [first?.sectionReference, distinctive, tokens((chunks[chunks.length - 1]?.sourceTextExcerpt || '')).slice(0, 2).join(' ')].filter(Boolean).join(' ')
  const cases = [
    { kind: 'title_identity', query: record.documentTitle, expectedCandidateId: record.candidateId, expectedMatch: true, expectedRegion: 'source identity', rankThreshold: 1, expectedSourceSha256: record.SHA256, expectedAuthorityRole: record.disposition, rationale: 'Title query is ranked against the complete batch corpus and must resolve to this source.' },
    { kind: 'substantive_content', query: substantiveQuery, expectedCandidateId: record.candidateId, expectedMatch: substantiveQuery.length > 0, expectedRegion: first?.sectionReference || 'first extracted region', rankThreshold: 1, expectedSourceSha256: record.SHA256, expectedAuthorityRole: record.disposition, rationale: 'A source-derived substantive query must resolve to the expected source in the corpus.' },
    { kind: 'structure_or_table', query: structureQuery, expectedCandidateId: record.candidateId, expectedMatch: true, expectedRegion: first?.sectionReference || structureQuery, rankThreshold: 1, expectedSheet: first?.extensions?.worksheet, expectedSourceSha256: record.SHA256, expectedAuthorityRole: record.disposition, rationale: 'A deterministic page window or workbook table locator remains citable.' },
    { kind: 'numeric_or_form', query: numericQuery, expectedCandidateId: record.candidateId, expectedMatch: tokens(numericQuery).length > 0, expectedRegion: numericChunk?.sectionReference || 'source structure', expectedSheet: record.expectedFileType === 'XLSX' ? numericEvidence?.worksheetPath : undefined, expectedCellRefs: record.expectedFileType === 'XLSX' ? [numericCell] : undefined, expectedSourceSha256: record.SHA256, expectedAuthorityRole: record.disposition, expectedWorkbookEvidence: record.expectedFileType === 'XLSX' ? numericEvidence : undefined, rankThreshold: 1, rationale: 'Numeric/form evidence is checked against the corpus, structural locator, role, and exact source SHA.' },
    { kind: 'authority_support_boundary', query: record.documentTitle + ' ' + (record.disposition || ''), expectedCandidateId: record.candidateId, expectedMatch: true, expectedRegion: 'source identity and approved role', expectedSourceSha256: record.SHA256, expectedAuthorityRole: record.disposition, rankThreshold: 1, rationale: 'Approved authority/support metadata remains bound to the selected source and is not upgraded.' },
    { kind: 'wrong_source_empty_match', query: 'qzxvblorfmivak-unmatched', expectedCandidateId: null, expectedMatch: false, expectedRegion: 'no matching corpus evidence', rankThreshold: 1, rationale: 'A nonsense token remains a basic no-match control and is not counted as confusion-set coverage.' },
  ]
  for (const slot of getConfusionSlots(record, allProcessed)) {
    const partner = allProcessed.find((item) => item.record.pilotSlot === slot && item.chunks.length)
    if (partner) cases.push({ kind: 'wrong_source_topic_negative', query: partner.record.documentTitle + ' ' + tokens(partner.chunks.find((chunk) => chunk.sourceTextType === 'actual_extracted_source_text')?.sourceTextExcerpt || '').slice(0, 3).join(' '), expectedCandidateId: partner.record.candidateId, negativeAgainstCandidateId: record.candidateId, expectedMatch: true, expectedRegion: 'confusion-set source: ' + slot, rankThreshold: 1, expectedSourceSha256: partner.observedSha256, expectedAuthorityRole: partner.record.disposition, rationale: 'A plausible overlapping domain query must resolve to the intended source and not accept the competing source as support.' })
  }
  return cases.map((testCase, index) => {
    const query = testCase.query || 'source'
    const q = tokens(query)
    const ranked = allChunks.map((chunk) => ({ chunkId: chunk.chunkId, sourceId: chunk.sourceId, score: q.filter((term) => chunk.normalizedSearchText.includes(term)).length + tokens(chunk.topic || '').filter((term) => q.includes(term)).length * 3 + (chunk.topic && normalize(query).toLowerCase() === normalize(chunk.topic).toLowerCase() ? 10 : 0) + (q.length >= 5 && chunk.normalizedSearchText.includes(normalize(query).toLowerCase()) ? 20 : 0) + (chunk.sectionReference === testCase.expectedRegion ? 8 : 0) + (testCase.expectedSheet && chunk.extensions?.worksheet === testCase.expectedSheet ? 8 : 0) })).filter((item) => item.score > 0).sort((a, b) => b.score - a.score || a.chunkId.localeCompare(b.chunkId))
    const top = ranked[0]
    ...
    const actualSource = allProcessed.find((item) => item.record.candidateId === selected?.sourceId)
    const selectedSourceSha256 = actualSource?.observedSha256 || null
    const selectedChunkSourceSha256 = selected?.extensions?.sourceSha256 || null
    const selectedAuthorityRole = selected?.extensions?.authoritySupportRole || actualSource?.record.disposition || null
    const shaMatches = !testCase.expectedSourceSha256 || (selectedSourceSha256 === testCase.expectedSourceSha256 && selectedChunkSourceSha256 === testCase.expectedSourceSha256)
    const roleMatches = !testCase.expectedAuthorityRole || (selectedAuthorityRole === testCase.expectedAuthorityRole && actualSource?.record.disposition === testCase.expectedAuthorityRole)
    ...
    const selectedNumericEvidence = testCase.expectedWorkbookEvidence && actualSource?.numericEvidence ? actualSource.numericEvidence : null
    const workbookEvidenceMatch = !testCase.expectedWorkbookEvidence || Boolean(selected?.extensions?.worksheet === testCase.expectedWorkbookEvidence.worksheetPath && selected.extensions?.tableBlock?.cellRefs?.includes(testCase.expectedWorkbookEvidence.cellRef) && selectedSourceSha256 === testCase.expectedWorkbookEvidence.sourceSha256 && selectedNumericEvidence?.cellRef === testCase.expectedWorkbookEvidence.cellRef && selectedNumericEvidence?.value === testCase.expectedWorkbookEvidence.value && selectedNumericEvidence?.formula === testCase.expectedWorkbookEvidence.formula)
    const pass = testCase.kind === 'wrong_source_topic_negative' ? Boolean(expectedTop && wrongSourceExcluded && citationResolves && shaMatches && roleMatches) : Boolean(testCase.expectedMatch ? expectedTop && citationResolves && structuralMatch && shaMatches && roleMatches && workbookEvidenceMatch : !top)
    const expectedWorkbookEvidence = testCase.expectedWorkbookEvidence ? { sheetName: testCase.expectedWorkbookEvidence.sheetName, worksheetPath: testCase.expectedWorkbookEvidence.worksheetPath, cellRef: testCase.expectedWorkbookEvidence.cellRef, expectedStoredValue: testCase.expectedWorkbookEvidence.value, expectedFormula: testCase.expectedWorkbookEvidence.formula, selectedStoredValue: selectedNumericEvidence?.value ?? null, selectedFormula: selectedNumericEvidence?.formula ?? null, sourceSha256: testCase.expectedWorkbookEvidence.sourceSha256, expectedAuthoritySupportRole: testCase.expectedWorkbookEvidence.expectedAuthoritySupportRole, citationCoordinate: testCase.expectedWorkbookEvidence.citationCoordinate } : null
    return { testId: record.candidateId + '-smoke-' + (index + 1), testKind: testCase.kind, query, expectedCandidateId: testCase.expectedCandidateId, negativeAgainstCandidateId: testCase.negativeAgainstCandidateId || null, expectedMatch: testCase.expectedMatch, expectedRankThreshold: testCase.rankThreshold || 1, expectedStructuralRegion: testCase.expectedRegion, expectedSheet: testCase.expectedSheet || null, expectedCellRefs: testCase.expectedCellRefs || [], expectedSourceSha256: testCase.expectedSourceSha256 || null, expectedAuthorityRole: testCase.expectedAuthorityRole || null, expectedWorkbookEvidence, topRank: top ? 1 : null, topChunkId: top?.chunkId || null, topSourceId: top?.sourceId || null, topScore: top?.score || 0, selectedSourceSha256, selectedChunkSourceSha256, selectedAuthorityRole, citationTarget: selected?.citations?.[0] || null, citationResolves, shaMatches, lineageMatches: Boolean(selected && actualSource && selected.sourceId === actualSource.record.candidateId && selectedChunkSourceSha256 === selectedSourceSha256), supportRolePreserved: roleMatches, roleMatches, structuralMatch, workbookEvidenceMatch, wrongSourceExcluded, pass, rationale: testCase.rationale }
  })
}
...
const main = async () => {
  ...
  for (const generatedFile of ['batch-manifest.json', 'source-inventory.json', 'extraction-output.json', 'chunk-manifest.json', 'source-index-candidates.json', 'retrieval-smoke-tests.json', 'external-artifact-manifest.json', 'review-packet.json', 'review-packet.md', 'unresolved-issues-summary.md', 'validation-report.json', 'a1-quarantine-evidence.md']) await fs.rm(path.join(outputRoot, generatedFile), { force: true })
  const processed = []
  const indexes = []
  ...
  for (const record of records) {
    ...
    try {
      ...
      const numericSheet = record.expectedFileType === 'XLSX' ? extraction.sheets.find((sheet) => !sheet.contentInventory?.substantiveNonCellContentMayExist && sheet.cells.some((cell) => cell.formula !== null || /^-?\d+(\.\d+)?$/.test(String(cell.value)))) : null
      const numericCellEvidence = numericSheet?.cells.find((cell) => cell.formula !== null || /^-?\d+(\.\d+)?$/.test(String(cell.value)))
      const numericEvidence = numericSheet && numericCellEvidence ? { sheetName: numericSheet.name, worksheetPath: numericSheet.worksheetPath, cellRef: numericCellEvidence.ref, value: numericCellEvidence.value, formula: numericCellEvidence.formula, sourceSha256: observedSha256, expectedAuthoritySupportRole: record.disposition, citationCoordinate: 'Worksheet ' + numericSheet.name + ', cell ' + numericCellEvidence.ref } : null
      const smoke = []
      ...
      const sourceIndex = {
        ...
        extensions: { acquisitionCandidateId: record.candidateId, rawSourcePath: record.localPath, rawSourceSha256: observedSha256, issuer: record.publisher, sourceUrl: record.initialUrl, acquisitionManifestPath: manifestPath, authoritySupportRole: record.disposition, structuredEvidence, rightsStatus },
      }
  ...
  const smokeTests = processed.map((item) => { const tests = corpusSmokeTestsFor(item.record, chunks, processed); return { candidateId: item.record.candidateId, sourceFamilyId: item.metadata.sourceFamilyId, tests, allPassed: tests.every((test) => test.pass) } })
  ...
  const checks = {
    ...
    retrievalShaAssertions: smokeTests.every((source) => source.tests.filter((test) => test.expectedMatch).every((test) => test.expectedSourceSha256 && test.shaMatches && test.lineageMatches)),
    retrievalRoleAssertions: smokeTests.every((source) => source.tests.filter((test) => test.expectedMatch).every((test) => test.expectedAuthorityRole && test.roleMatches)),
    a2NumericAssertions: smokeTests.flatMap((source) => source.tests).filter((test) => test.testKind === 'numeric_or_form' && test.expectedWorkbookEvidence).every((test) => test.workbookEvidenceMatch && test.expectedWorkbookEvidence.cellRef && Object.prototype.hasOwnProperty.call(test.expectedWorkbookEvidence, 'expectedFormula')),
    confusionSetNegatives: smokeTests.flatMap((source) => source.tests).filter((test) => test.testKind === 'wrong_source_topic_negative').length >= Math.min(7, records.length) && smokeTests.flatMap((source) => source.tests).filter((test) => test.testKind === 'wrong_source_topic_negative').every((test) => test.pass && test.wrongSourceExcluded),
  ...
  for (const item of processed) {
    const source = inventoryItems.find((candidate) => candidate.sourceId === item.record.candidateId)
    const group = extractionGroups.find((candidate) => candidate.sourceId === item.record.candidateId)
    const index = indexes.find((candidate) => candidate.source.sourceId === item.record.candidateId)
    const smoke = smokeTests.find((candidate) => candidate.candidateId === item.record.candidateId)
    for (const [artifactType, value] of [['extraction', group], ['chunk-manifest', { sourceId: item.record.candidateId, chunks: chunks.filter((chunk) => chunk.sourceId === item.record.candidateId) }], ['source-index', index], ['retrieval-smoke-tests', smoke]]) externalArtifacts.push(await writeExternalJson(externalProcessingRoot, item.record.candidateId, artifactType, value, source, acquisition.generatedAt))
    source.externalArtifactTypes = substantiveArtifactTypes
    source.externalArtifactManifestPath = 'data/processed/review_packages/' + batchId + '/external-artifact-manifest.json'
  }
  ...
  const batch = { schemaVersion: '1.0', batchId, batchName: 'Controlled scaled review-only batch ' + batchId, status: 'review_pending', createdAt: acquisition.generatedAt, updatedAt: acquisition.generatedAt, projectName: 'Document Processor', projectPurpose: 'Review-only processing candidates bound to a controlled acquisition wave.', sourceFamilies: families, sourceFiles: inventoryItems, processingIntent: { mode: 'scaled_review_only', targetDomains: families.map((family) => family.domainId), pipelineStages: ['identity/provenance', 'extraction', 'structure_detection', 'segmentation/chunking', 'structured_evidence', 'metadata_classification', 'source_index_candidate', 'retrieval_smoke_test', 'validation', 'review_package'], smallPilot: false, learnerFacingBlocked: true, appExportRequested: false, ragReadinessRequested: false, reviewStrategy: 'exception_first', notes: 'Only records explicitly admitted by the acquisition manifest were processed; quarantined records remained outside the processing queue.' }, boundaries: { rawSourceRoot: acquisition.intakeRoot, rawMaterialExternal: true, substantiveArtifactsExternal: true, externalProcessingRoot, gitExcludesRawFiles: true, gitExcludesSubstantiveArtifacts: true, noLearnerFacingPromotion: true, noCanonicalPromotion: true, acquisitionManifestPath: manifestPath }, expectedOutputs: ['source_inventory', 'extraction_output', 'chunk_manifest', 'source_index_candidates', 'retrieval_smoke_tests', 'review_packet', 'validation_report', 'external_artifact_manifest'], reviewStatus: { defaultStatus: 'draft_candidate', reviewStrategy: 'exception_first', reviewFocus: 'exceptions_and_stratified_sample', promotionGate: 'separate human decision required; no automatic promotion', learnerFacingAllowed: false, appReadyAllowed: false, reviewPacketRequired: true }, extensions: { processingRunId, acquisitionManifestPath: manifestPath, authorizedProcessingScope: 'ADMIT_TO_PROCESSING_QUEUE only', quarantinedRecordCount: quarantineRecords.length, a1Excluded: !records.some((record) => record.pilotSlot === 'A1') } }
  ...
  const rightsIssue = records.some((record) => record.candidateId === 'naic-accounting-publications-appm-2026') ? { issueId: 'naic-accounting-publications-appm-2026-rights-distribution-control', severity: 'high', issueType: 'rights_distribution_control', sourceId: 'naic-accounting-publications-appm-2026', message: 'A3 distribution/software-integration restriction requires rights and storage review; no legal conclusion is made.', recommendedAction: 'Keep substantive A3 text external/private pending rights review; retain only non-content processing evidence in public Git.' } : null
  const reviewItems = processed.map((item) => ({ stableId: item.record.candidateId + '--review-item', sourceId: item.record.candidateId, sourceFamilyId: item.metadata.sourceFamilyId, domainId: item.metadata.domainId, documentType: item.metadata.documentType, sourceReference: item.record.initialUrl, sourcePath: item.record.localPath, pageReference: item.record.expectedFileType === 'PDF' ? 'pp. 1-' + item.pageCount : 'Workbook', sectionReference: item.record.expectedFileType === 'XLSX' ? 'Workbook sheets' : 'Page-aware extraction', outcome: item.outcome, exceptionCode: item.exceptionCode, summary: item.reason, confidence: item.outcome === 'CLEAN_REVIEW_CANDIDATE' ? 'high' : 'medium', reviewFlags: item.outcome === 'CLEAN_REVIEW_CANDIDATE' ? [] : [item.exceptionCode], learnerFacingEligible: false, appReadyEligible: false, rightsStatus, externalArtifactManifestPath: 'data/processed/review_packages/' + batchId + '/external-artifact-manifest.json', notes: 'Review-only candidate; no promotion or downstream eligibility is implied.' }))
  const exceptionItems = reviewItems.filter((item) => item.outcome === 'HUMAN_REVIEW_REQUIRED').map((item) => item.sourceId)
  const cleanItems = reviewItems.filter((item) => item.outcome === 'CLEAN_REVIEW_CANDIDATE').map((item) => item.sourceId)
  const cleanSampleIds = batchId === 'acquisition-pilot-2026-09-02' ? cleanItems : cleanItems.slice().sort().slice(0, Math.max(3, Math.ceil(cleanItems.length * 0.2)))
  const review = { schemaVersion: '1.0', packetId: 'review-packet-' + batchId, batchId, generatedAt: acquisition.generatedAt, reviewMode: 'exception_first', batchSummary: { batchName: batch.batchName, sourceFamilies: families.map((family) => family.sourceFamilyId), processingIntent: 'Low-touch review-only processing for admitted raw sources.', sourceFileCount: records.length, extractedItemCount: chunks.length, exceptionCount: flags.length, summary: 'All admitted sources were processed through review-only extraction, structure, chunking, source-index candidate, retrieval smoke test, and validation.', notes: JSON.stringify(summary) }, sourceFilesProcessed: inventoryItems.map((item) => ({ sourceId: item.sourceId, filename: item.filename, filePath: item.filePath, sourceReference: item.sourceReference, sourceFamilyId: item.sourceFamilyId, domainId: item.domainId, documentType: item.documentType, processingStatus: 'review_pending', pageCount: item.pageCount, issueCount: flags.filter((flag) => flag.sourceId === item.sourceId).length, rightsStatus, notes: item.notes })), extractedItems: reviewItems, cleanReviewSample: { sampleType: 'CLEAN_REVIEW_CANDIDATE', sourceIds: cleanSampleIds }, exceptionReviewSample: { sampleType: 'HUMAN_REVIEW_REQUIRED', sourceIds: exceptionItems }, requiredHumanDecisions: decisions.concat(rightsIssue ? [rightsIssue] : []), exceptionsAndFlags: flags, citationIssues: [], unresolvedIssues: flags.map((flag) => ({ issueId: flag.flagId, severity: flag.severity, issueType: flag.flagType, sourceId: flag.sourceId, outcome: 'HUMAN_REVIEW_REQUIRED', exceptionCode: flag.flagType.toUpperCase(), message: flag.message, recommendedAction: 'Review source-specific evidence before any future promotion decision.' })).concat(rightsIssue ? [rightsIssue] : []), promotionRecommendation: { status: 'not_recommended', reason: 'This is a controlled review-only batch; no canonical promotion is authorized.', recommendedNextStep: 'Inspect the exception review sample and the stratified clean review sample.', targetExport: 'review_packet' }, learnerFacingStatus: { ready: false, reason: 'No learner-facing use is authorized.', statusText: 'not approved' }, ragReadiness: { ready: false, reason: 'No RAG eligibility is authorized.', indexableItemCount: 0, notes: 'Source-index candidates are review-only.' }, appExportReadiness: { ready: false, reason: 'No app-ready export is authorized.', targetExports: [], notes: 'No promotion was performed.' }, reviewerNotes: 'Clean sample contains only CLEAN_REVIEW_CANDIDATE records. Exceptions are routed to human review. Substantive rights-controlled artifacts remain external/private.', extensions: { processingRunId, acquisitionManifestPath: manifestPath, summary } }
  ...
  const a1Evidence = a1 ? ['# A1 quarantine evidence packet', '', '- Candidate ID: ' + a1.candidateId, '- Approved expected identity/status: PBR plenary adopted amendments, current edition / CURRENT AUTHORITY - DOWNLOAD', '- Exact downloaded filename: ' + a1.actualFilename, '- Final URL: ' + a1.finalUrl, '- SHA-256: ' + a1.SHA256, '- Quarantine path: ' + a1.localPath, '- Observed first-page identity evidence: Meeting Materials: Life Insurance and Annuities (A) Committee; July 14, 2025; Attachment A - Life Amendments to the 2026 Valuation Manual for the Consideration of the Life Insurance and Annuities (A) Committee.', '- Observed title/date/version: July 14, 2025 committee meeting attachment concerning amendments to the 2026 Valuation Manual.', '- Exact contradiction: the payload identity does not present as the catalog-described current adopted-amendment edition.', '- Possible issue classes for human adjudication: catalog metadata issue; URL/current-resource issue; document-version issue; amendment incorporation issue; other.', '- Human reviewer needs to compare the official page listing, current-resource link, adoption/effective metadata, and this payload identity.', '- Raw payload authenticity/integrity evidence: PDF signature and EOF validation passed; SHA remained unchanged when the payload was moved to quarantine.', '', 'This packet does not resolve the contradiction or make a final authority/status determination. A1 was not processed, promoted, or redownloaded.'].join('\n') : null
  const externalManifest = { schemaVersion: '1.0', manifestId: 'external-artifacts-' + batchId, processingRunId, generatedTimestamp: acquisition.generatedAt, externalProcessingRoot, rightsStorageStatuses: [rightsStatus], reviewOnly: true, artifacts: externalArtifacts }
  const sanitizedExtraction = { ...extraction, sourceGroups: extraction.sourceGroups.map(redactExtractionGroup), substantiveTextExternal: true }
  const sanitizedChunkFile = { ...chunkFile, chunks: chunkFile.chunks.map(redactChunk), substantiveTextExternal: true }
  const sanitizedIndexes = indexes.map((index) => redactSourceIndex(index, externalArtifacts.filter((item) => item.sourceId === index.source.sourceId)))
  const sanitizedSmoke = { schemaVersion: '1.0', processingRunId, tests: smokeTests.map(redactSmoke), summary: { sourceCount: records.length, passedSourceCount: smokeTests.filter((test) => test.allPassed).length, testCount: smokeTests.reduce((sum, source) => sum + source.tests.length, 0), substantiveQueriesExternal: true } }
  await writeJson(path.join(outputRoot, 'batch-manifest.json'), batch)
  await writeJson(path.join(outputRoot, 'source-inventory.json'), inventory)
  await writeJson(path.join(outputRoot, 'extraction-output.json'), sanitizedExtraction)
  await writeJson(path.join(outputRoot, 'chunk-manifest.json'), sanitizedChunkFile)
  await writeJson(path.join(outputRoot, 'source-index-candidates.json'), { schemaVersion: '1.0', processingRunId, reviewOnly: true, promotionStatus: 'not_promoted', sourceIndexes: sanitizedIndexes, substantiveTextExternal: true })
  await writeJson(path.join(outputRoot, 'retrieval-smoke-tests.json'), sanitizedSmoke)
  await writeJson(path.join(outputRoot, 'external-artifact-manifest.json'), externalManifest)
  await writeJson(path.join(outputRoot, 'review-packet.json'), review)
  await writeText(path.join(outputRoot, 'review-packet.md'), reviewMd)
  await writeText(path.join(outputRoot, 'unresolved-issues-summary.md'), unresolved)
  await writeJson(path.join(outputRoot, 'validation-report.json'), report)
  if (a1Evidence) await writeText(path.join(outputRoot, 'a1-quarantine-evidence.md'), a1Evidence)
