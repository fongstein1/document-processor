...
const main = async () => {
  ...
  await writeJson(path.join(outputRoot, 'extraction-output.json'), extraction)
  await writeJson(path.join(outputRoot, 'chunk-manifest.json'), chunkFile)
  // Release duplicate in-memory representations before serializing the review index.
  extraction.sourceGroups.length = 0
  chunkFile.chunks.length = 0
  chunks.length = 0
  for (const item of processed) { item.chunks = []; item.structuredEvidence = []; item.emptyPageDetails = [] }
  await writeJson(path.join(outputRoot, 'source-index-candidates.json'), { schemaVersion: '1.0', processingRunId, reviewOnly: true, promotionStatus: 'not_promoted', sourceIndexes: indexes })
  await writeJson(path.join(outputRoot, 'retrieval-smoke-tests.json'), { schemaVersion: '1.0', processingRunId, tests: smokeTests, summary: { sourceCount: records.length, passedSourceCount: smokeTests.filter((test) => test.allPassed).length, testCount: smokeTests.reduce((sum, source) => sum + source.tests.length, 0) } })
