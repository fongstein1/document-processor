...
const main = async () => {
  ...
  if (report.summary.totalAdmitted !== 7 || report.summary.processedSuccessfully !== 7) fail('Live acquisition pilot source count changed unexpectedly.')
  ...
  if (smoke.tests.some((source) => source.tests.length < 6 || !source.tests.find((test) => test.testKind === 'wrong_source_negative' && test.pass))) fail('Retrieval regression coverage is incomplete or the negative guard failed.')
