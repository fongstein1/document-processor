...
assert.throws(() => validateGitSafeArtifact({ artifactType: 'negative child text', value: { childId: 'c', sourceTextExcerpt: 'substantive semantic child text' } }))
assert.throws(() => validateGitSafeArtifact({ artifactType: 'negative header text', value: { headerContextId: 'h', headerText: 'Category Mortality Factor' } }))
assert.throws(() => validateGitSafeArtifact({ artifactType: 'negative gold rationale', value: { caseId: 'case', goldRationale: 'This source passage governs the answer.' } }))
assert.throws(() => validateGitSafeArtifact({ artifactType: 'negative BM25 vocabulary', value: { bm25Vocabulary: ['mortality', 'factor'] } }))
assert.throws(() => validateGitSafeArtifact({ artifactType: 'negative context package', value: { contextId: 'ctx', contextPackageText: 'substantive expanded evidence' } }))
...
console.log(JSON.stringify({ tests: 'semantic-evidence-unit-correction', result: 'PASS', fixtures: { rareTermIdf: 'PASS', repeatedTermTf: 'PASS', lengthNormalization: 'PASS', regulatoryIdentifiers: 'PASS', meaningfulPhrases: 'PASS', deterministicTies: 'PASS', parentRerankChangesRanking: 'PASS', pdfSemanticBoundaries: 'PASS', xlsxBoundedRanges: 'PASS', xlsxHeaderContext: 'PASS', roleAwareContext: 'PASS', expectationLeakage: 'PASS', rightsProjection: 'PASS', rightsSemanticPrivateFields: 'PASS', a3Hierarchy: 'PASS' }, deterministicRerun }, null, 2))
