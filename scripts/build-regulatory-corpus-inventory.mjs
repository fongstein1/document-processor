...
const targetDefinitions = [
  ...
  ['vm-01', 'VM-01 Definitions', 'valuation_manual_pdfs', 'P0', 'current', 'The current 2026 VM-01 terminology layer contains 98 exact-text definition units. Its two independent-review blockers are corrected without source-evidence changes; it remains review-only and not promoted.', 'Run the narrow final review of source-explicit definedTerms and the case-level retrieval artifact, then record a separate promotion decision only if approved.'],
...
const buildCompleteness = (inventory) => {
  ...
  return [
    ...
    `- VM-01 promotion status: ${s.vm01PromotionStatus}; narrow final review remains required after the two targeted blocker corrections.`,
