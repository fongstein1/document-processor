...
const targetDefinitions = [
  ...
  ['vm-20', 'VM-20 Requirements for Principle-Based Reserves for Life Products', 'valuation_manual_pdfs', 'P0', 'current', 'VM-20 now has a 247-chunk review-only coverage set: 72 current-manual chunks across the existing slices and expanded batches, plus 175 separately labeled companion-guidance chunks; it is still not a complete current chapter package.', 'Review the dedicated VM-20 package, then complete authoritative Sections 4, 5, and 9 plus Appendices 1-2 and structured current tables.'],
...
const buildCompleteness = (inventory) => {
  const s = inventory.summary
  return [`# Corpus Completeness Report`, '', `As of ${inventory.generatedAt}, the repository contains a substantial review-only regulatory evidence base but is not production-complete and is not Copilot-export ready.`, '', '## Counts', '', `- Source documents inventoried: **${s.sourceDocumentsInventoried}**`, `- Documents with declared external raw source: **${s.sourceRecordsWithExternalManifest}**`, `- Documents reviewed / review artifacts matched: **${s.reviewIndexes} review indexes**`, `- Canonical source packages: **${s.canonicalSourcePackages}**`, `- Canonical chunks: **${s.canonicalChunks}**`, `- Sources awaiting canonicalization: **${s.sourcesAwaitingCanonicalization}**`, `- Sources awaiting human review: **${s.sourcesAwaitingHumanReview}**`, `- Review-artifact-only sources: **${s.reviewArtifactOnlySources}**`, `- Candidate relationships: **${s.candidateRelationships}**`, `- Promoted relationships: **${s.promotedRelationships}**`, '', '## VM-20 coverage checkpoint', '', '- Current-manual VM-20 coverage: 72 chunks across the existing framework/Section 3.C packages and the expanded reviewed batches 004, 005, and 007-012.', '- VM-20 companion coverage: 175 exact-text chunks from the reviewed 2020 practice-note wave, separately labeled non-binding and historical.', '- Dedicated review package: `data/processed/review_packages/vm20-canonical-coverage-review-package.json` and `.md`.', '- Remaining VM-20 gaps: complete authoritative Sections 4, 5, and 9; Appendices 1-2; and structured current prescribed tables.', '', '## Interpretation', '', '- “Canonical” means a review-only package exists in the tracked source-index POC, not that it is approved for downstream use.', '- “Reviewed” means a tracked review index or self-review exists; it does not prove that all source text was canonicalized.', '- “Missing” in a target assessment means no supporting batch, canonical package, or tracked review evidence was found in the current repository snapshot.', '- Source-text fidelity is explicit at package/chunk level; summary-only or review-derived material must not be labeled verbatim source text.', '', '## Copilot handoff posture', '', '- Export eligibility is false for every inventory record.', '- Approved export version and exclusion reason are reserved for a later explicit promotion decision.', '- Copilot should consume a generated export from the canonical corpus, never become the canonical source.', ''].join('\n')
}
