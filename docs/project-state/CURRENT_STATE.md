...
the AG 09 planning layer validate cleanly.
...
The AG 09 planning layer is now validated and ready for the family wave
(`batch-084` through `batch-087`), which should remain review-only in
ignored working storage until the review-handoff artifacts are created.
