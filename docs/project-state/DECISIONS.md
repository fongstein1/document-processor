...
2026-07-22: The Regulation 213 relationship proof of concept uses formal amendment titles and explicit tracked review-artifact links for documentary candidates only. Every candidate must carry evidence, confidence, generation rule, caveat, direction, pending review decision, and not-promoted status. Chronology alone cannot establish supersession, an FAQ cannot be treated as binding authority, and a reprint cannot be treated as controlling without human review.
2026-08-26: Promote only the six reviewed 2026 VM-20 current-manual prose packages (149 chunks). Preserve their source text, hierarchy, citations, and retrieval behavior; keep the 2020 practice note, structured tables, relationship candidates, and all downstream export permissions outside the promotion.
2026-08-26: Keep VM-20 Appendix 2 tables in a separate generic structured-table dataset. Ingest only official current-page workbooks, retain every workbook/sheet/cell locator and displayed precision, mark unavailable Tables B-E2 without inference, and require independent review plus a separate promotion decision.
2026-08-26: Treat workbook Average rows as preserved source summary statistics, never as prescribed regulatory dimensions. For Table J, record official-workbook association separately from Manual table identity: the current benchmark swap-spread column has no Manual Table J assignment, while the long-term benchmark swap-spread column carries the Manual Table J identity. Keep the corrected dataset review-only until the narrow final gate and a separate promotion decision.
2026-08-26: Promote only the independently approved available VM-20 Appendix 2 structured-table scope for Tables A, F, G, H, I, J, and K. Preserve all 7,022 source values, 891 rows, workbook hashes, version/effective-date metadata, Average-row ineligibility, Table J authority boundaries, and unavailable Tables B-E2. Canonical promotion does not grant learner, app, RAG, or Copilot eligibility.
2026-08-26: Represent current 2026 VM-01 as a distinct definition-aware canonical review candidate with one intact retrieval unit per source definition. Preserve exact extracted evidence, record transparent lookup-only text-layer spacing corrections, admit only source-explicit aliases and acronym expansions, and keep all relationship candidates pending. Canonical package creation does not promote VM-01 or authorize downstream export.
2026-08-26: Close the two VM-01 `APPROVE WITH FIXES` blockers without reopening source processing. Restrict canonical `definedTerms` to the formal term and source-explicit aliases, retain generated normalization only in existing retrieval metadata, preserve a case-level focused evaluation and local PDF hash confirmation for review, and require a narrow final independent decision before promotion.
