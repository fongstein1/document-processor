# Controlled AG 17 Extraction Plan
...
AG 17 is a one-page CRVM guideline on non-level death benefits. The
page is compact enough to process as a single review-only batch, but the
...
- Primary source file: `AG 17 - Guideline for Calculation of Commissioners' Reserve Valuation Method When Death Benefits are Not Level.pdf`
- Source family: `actuarial_guidelines`
- Source reference: `Actuarial Guideline XVII`
- Domain: `naic_regulatory`
- Raw source root: `D:\Work\AI Projects\NAIC Valuation Manual Course`
- Confirmed page range: page 1
- page 1 carries the full CRVM guideline, including the non-level death-benefit explanation.
- Source posture: historical guideline / caveat-first CRVM interpretation; no withdrawn notice observed in the scanned copy

### Boundaries

- Keep planning focused on this single AG 17 PDF.
- Do not widen into AG 18 or later guideline files.
...
| --- | --- | --- | --- | --- | --- |
| `ag17-crvm-nonlevel-death-benefits` | CRVM non-level death-benefit interpretation | 1 | Medium | regulatory_requirement, definition_or_terminology, formula_context, cross_reference_mapping, background_content, requires_human_interpretation, boundary_control_window, historical_guidance | equivalent level renewal amount, average amount of insurance, Standard Valuation Law, Section 5-c, 1980 CSO Table, jumping juvenile |

## Proposed Batch Sequence

| Planned batch | Section / topic | Page target | Rationale | Review complexity |
| --- | --- | --- | --- | --- |
| `batch-095` | AG 17 non-level death-benefit guideline | 1 | Capture the full one-page guideline as a single review-only slice and stop at the page boundary. | Medium |
...
- Express duty, permitted treatment, or required CRVM calculation language in
  the non-level death-benefit context.

### What counts as terminology or interpretation

- Equivalent level renewal amount, average amount of insurance, non-level death
  benefit, and Section 5-c phrasing when they explain rather than direct.
...
- References to the Standard Valuation Law, Section 5-c, the 1980 CSO Table,
  or related CRVM terminology when they appear as mapping clues.

### What requires human actuarial interpretation

- Whether AG 17 should stay indexed as historical caveat-first guidance or be
  linked only as a non-level death-benefit note.
...
- Add lightweight validation for `config/ag17-batch-plan.json`.
- Keep `npm run check` aware of the plan file so the scaffold proves the plan
  exists and stays well formed.
- When the real batch starts, keep `scripts/ag17-batch-definitions.mjs`
  synchronized with the planned `batch-095` slice.
- Add future checks for historical-guidance, caveat-first, and no-promotion
  guardrails.
- Keep the plan separate from AG 16, later guideline files,
  `docs/review/ag16_review_index.md`, `docs/review/ag15_review_index.md`,
