...
AG 47 is Actuarial Guideline XLVII. It was processed as a single-batch
review-only wave against the active 4-page
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `batch-153` | `pages 1-4` | Claim-reserve company-experience, credibility, and lag-period framing | Keep the opening guidance and claim-reserve calculation context together as one narrow review-only slice and stop cleanly at the AG 48 boundary. | `regulatory_requirement`; `definition_or_terminology`; `reserve_method_structure`; `calculation_structure`; `formula_context`; `company_or_prudent_estimate_assumption`; `cross_reference_mapping`; `background_content`; `boundary_control_window`; `requires_human_interpretation` | Encoding-noisy OCR layer; page-image wording backstop; line references were not available; claim-reserve boundary; AG 48 remains out of scope. | `reasonable_with_minor_cautions` | passed | `not separately recorded` |
...
- Line references were not available, so page locators remain the primary
  review anchor.
- The opening guidance, claim-reserve calculation language, credibility
  language, and lag-period language should stay review-only until a human
  confirms the wording against the page image.
- The claim-reserve passages should not be over-interpreted as broader policy
  language than the source text provides.
- AG 48 remains out of scope and should not be blended into this wave.
- Cross references to the 2012 GLTD Valuation Table, company experience,
  credibility, and lag-period framing should be mapped but not over-
  interpreted.
...
- Are the claim-reserve calculation passages separated from background?
- Are credibility and lag-period passages mapped but not over-interpreted?
- Is the AG 48 boundary kept outside this wave?
...
1. Review batch-153 first because it establishes the opening guidance,
   claim-reserve calculation context, and AG 48 boundary slice.
...
- AG 47 self-review note: `docs/review/ag47_self_review.md`

## Review Notes

- The AG 47 review packet is intentionally narrow and review-only.
- The page-image wording backstop is the wording authority for this wave
  because line references were not expected.
- The source remains active and AG 48 remains out of scope.
