# AG 14 End-of-Sequence Review Index
...
This tracked index summarizes the ignored review packet for controlled AG 14
batch `batch-092`. It is a handoff summary only. The underlying batch outputs
...
commit that immediately preceded the AG 14 batch, `58ecce4`, not to the
ignored working files themselves.

## Overall AG 14 Extraction Status

- The controlled AG 14 sequence is complete.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `batch-092` | Actuarial Guideline XIV / AG 14 - Surveillance Procedure Regarding the Actuarial Opinion for Life and Health Insurers.pdf | pages 1-2; historical interim surveillance procedure | Historical surveillance procedure for actuarial-opinion review | This two-page interim procedure is intentionally limited to the full historical note and keeps the page-image confirmation caveat visible. | regulatory_requirement, reporting_requirement, documentation_expectation, governance_or_control_expectation, caveat_or_companion_guidance, cross_reference_mapping, requires_human_interpretation, boundary_control_window, historical_guidance | reasonable_with_minor_cautions | Page-image confirmation is still needed because the text layer is encoded/noisy, and the historical disposition should stay caveat-first. | passed | `58ecce4` |

## Higher-Caution Section

- AG 14 is a two-page historical surveillance procedure, not a withdrawn
  guideline.
- The page-image wording remains the best backstop because the text layer is
  encoded/noisy on both pages.
- The interim procedure framing should stay caveat-first and not be over-read
  as learner-facing policy text.
- The examples around actuarial opinion review, actuarial report support,
  IRIS ratios, model legislation, and confidentiality should remain review-only
  context.
...
- Is the historical status visible enough that no one mistakes it for active
  guidance?
- Are surveillance-procedure and documentation references mapped but not
  over-interpreted?
...
1. AG 14 pages 1-2 as a single historical note.
2. Confirm the page-image wording for the surveillance-procedure and
   documentation language.
3. Resolve the historical indexing disposition if a later cleanup step wants a
   broader surveillance-procedure link.
...
This AG 14 index should be reviewed alongside the AG 13 index, the VM chapter
...
A separate self-review note at `docs/review/ag14_self_review.md` records the
