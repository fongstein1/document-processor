...
- **vm-01 — VM-01 Definitions:** canonical_review_candidate. The current 2026 VM-01 terminology layer contains 98 exact-text definition units. Retrieval behavior and corpus integrity passed narrow review; byte-exact implementation snapshots now close the remaining code-handoff evidence gap. It remains review-only and not promoted. Next: Run only the final code-level review of the hashed implementation snapshots, then record a separate promotion decision only if approved.
