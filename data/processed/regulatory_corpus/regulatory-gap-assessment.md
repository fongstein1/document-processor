...
- **vm-01 — VM-01 Definitions:** reviewed_not_canonical. The 2026 manual is present and VM-01 is named in the supporting-wave plan, but no canonical VM-01 package is present. Next: Canonicalize definitions first because downstream VM answers depend on stable terms.
- **vm-20 — VM-20 Requirements for Principle-Based Reserves for Life Products:** canonical_partial. VM-20 now has a 324-chunk review-only coverage set: 149 current-manual chunks across the existing slices and new authoritative prose/appendix batches, plus 175 separately labeled companion-guidance chunks. Structured current Appendix 2 tables remain deferred. Next: Review the dedicated VM-20 package, then process the separate structured current-table milestone while preserving the prose/appendix coverage as review-only.
...
- **current-regulatory-tables — Current prescribed valuation, mortality, spread, and default tables:** reviewed_not_canonical. VM-20/21/22 reviewed material references tables, but no structured table corpus is present in the canonical layer. Next: Create a table-specific profile with row/column citations and version metadata; do not prose-chunk tables.
